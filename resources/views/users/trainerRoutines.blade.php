...
<x-app-layout>
    ...
    <x-slot name="header">
        ...
        @if (isset($userRoutine[0]->user->name) && isset($userRoutine[0]->user->surname))
            {{-- Título de la página --}}
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ 'Rutinas del entrenador' . $userRoutine[0]->user->name . ' ' . $userRoutine[0]->user->surname }}
            </h2>
    ...
    @if ($userRoutine)
        ...
        <div class="flex flex-col lg:flex-row flex-wrap py-2 px-2 md:px-5 my-4 gap-4 justify-strech">
            @foreach ($userRoutine as $routine)
