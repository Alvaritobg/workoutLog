function crearRutina(tieneSuscripcion) {

    if (tieneSuscripcion === 'true') {
        // Redirigir al usuario a la página de creación de rutinas
        window.location.href = "{{ route('routines.nueva') }}";
    } else {
        // Mostrar mensaje de error
        mostrarNotificacion('Necesitas tener una suscripción pagada activa para crear más rutinas.');
    }
}

function editarRutina(tieneSuscripcion, idRutina) {

    if (tieneSuscripcion === 'true') {
        // Redirigir al usuario a la página de creación de rutinas
        window.location.href = `{{ route('routines.edit',['id' => ${idRutina}] }}`;
    } else {
        // Mostrar mensaje de error
        mostrarNotificacion('Necesitas tener una suscripción pagada activa para editar rutinas.');
    }
}

function eliminarRutina(tieneSuscripcion, idRutina) {

    if (tieneSuscripcion === 'true') {
        // Redirigir al usuario a la página de creación de rutinas
        window.location.href = `{{ route('routines.destroy',['id' => ${idRutina}] }}`;
    } else {
        // Mostrar mensaje de error
        mostrarNotificacion('Necesitas tener una suscripción pagada activa para eliminar rutinas.');
    }
}
...
function mostrarNotificacion(mensaje, tipo = 'error') {
    const notificationContainer = document.getElementById('dynamicNotification');
